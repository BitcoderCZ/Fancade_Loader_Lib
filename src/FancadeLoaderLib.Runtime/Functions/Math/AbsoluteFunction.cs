...
public sealed class AbsoluteFunction : UnaryFunction
{
    public AbsoluteFunction(RuntimeTerminal input)
        : base(input)
    {
    }
    ...
    public override TerminalOutput GetTerminalOutput(byte3 terminalPos, IRuntimeContext context)
    {
        ...
        return new TerminalOutput(new RuntimeValue(MathF.Abs(Input.GetOutput(context).GetValue(context).Float)));
    }
}
