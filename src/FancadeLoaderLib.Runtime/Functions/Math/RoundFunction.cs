...
public sealed class RoundFunction : UnaryFunction
{
    public RoundFunction(RuntimeTerminal input)
        : base(input)
    {
    }
    ...
    public override TerminalOutput GetTerminalOutput(byte3 terminalPos, IRuntimeContext context)
    {
        ...
        return new TerminalOutput(new RuntimeValue(MathF.Round(Input.GetOutput(context).GetValue(context).Float)));
    }
}
