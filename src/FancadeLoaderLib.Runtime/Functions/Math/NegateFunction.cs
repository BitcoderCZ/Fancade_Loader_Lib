...
public sealed class NegateFunction : UnaryFunction
{
    public NegateFunction(RuntimeTerminal input)
        : base(input)
    {
    }
    ...
    public override TerminalOutput GetTerminalOutput(byte3 terminalPos, IRuntimeContext context)
    {
        ...
        return new TerminalOutput(new RuntimeValue(-Input.GetOutput(context).GetValue(context).Float));
    }
}
