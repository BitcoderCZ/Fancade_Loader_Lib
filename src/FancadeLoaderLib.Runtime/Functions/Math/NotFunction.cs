...
public sealed class NotFunction : UnaryFunction
{
    public NotFunction(RuntimeTerminal input)
        : base(input)
    {
    }
    ...
    public override TerminalOutput GetTerminalOutput(byte3 terminalPos, IRuntimeContext context)
    {
        ...
        return new TerminalOutput(new RuntimeValue(!Input.GetOutput(context).GetValue(context).Bool));
    }
}
