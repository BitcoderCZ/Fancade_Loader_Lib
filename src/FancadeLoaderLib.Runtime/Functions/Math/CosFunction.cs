...
public sealed class CosFunction : UnaryFunction
{
    public CosFunction(RuntimeTerminal input)
        : base(input)
    {
    }
    ...
    public override TerminalOutput GetTerminalOutput(byte3 terminalPos, IRuntimeContext context)
    {
        ...
        return new TerminalOutput(new RuntimeValue(MathF.Cos(Input.GetOutput(context).GetValue(context).Float)));
    }
}
